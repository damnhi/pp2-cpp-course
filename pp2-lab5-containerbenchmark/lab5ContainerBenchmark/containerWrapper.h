...
public:
    ...
    LazyContainerWrapper(const value_type [], std::size_t) {}
...
public:
    ...
    DequeWrapper(const value_type elements[], std::size_t size) {
            for (int i = 0; i < size; ++i) {
                impl_.push_back(elements[i]);
            }
        }


    void push_back(const value_type& element) override {impl_.push_back(element);}
    void push_front(const value_type& element) override {impl_.push_front(element);}
    ...
    void sort() override { std::sort(impl_.begin(), impl_.end());}

    void erase(std::size_t position) override { impl_.erase(impl_.begin()+position);}
