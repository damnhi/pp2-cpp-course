...
public:
    ...
    //sprawdz ponizsza metode:
    auto data() const { return data_;}
    char* c_str()const;
    static std::size_t instances();
    void assign(const char* new_text);
    bool equal_to(const SimpleString& second_string,bool case_sensitive=true) const;
    void append(const SimpleString& second_string);
    SimpleString substr(size_t pos = 0, size_t count = -1) const;
    int compare(const SimpleString& text1, bool case_sensitive=true) const;
